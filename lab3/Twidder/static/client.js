...
loginValidation = function (email, password) {
    ...
    PostFunctionAJAX("/signin", localStorage.getItem("token"), "Content-Type", "application/json", loginData, function() {
            if (this.success) {
                connectToSocket(localStorage.email);
...
connectToSocket = function (email) {
    var connection = new WebSocket('ws://127.0.0.1:5169/api');
    ...
    // Log messages from server. Is the user logged in elsewhere? Log it out!
    connection.onmessage = function(response) {
        if (response.data == "signOutNow") {
            // signOut();
            // or do you have to clear the local storage and loadview here?
            localStorage.clear();
            displayView();
    ...
    // add connection.onclose and/or connection.onerror?
    // have some sort of code that check if websocket connetion exists?
